...
void UpdateFromSystemSettings(blink::RendererPreferences* prefs,
  ...
  // Dchromium fork: force WebRTC to disable non-proxied UDP, blocking
  // host/STUN candidate gathering and preventing IP leak via WebRTC.
  // This matches the C版基线2 fingerprint (host=blocked, stun=blocked,
  ...
  // P0: VirtualEnvironmentManager — override accept_languages from profile.json.
  // VEM writes --dchromium-fork-accept-languages as a comma-separated list
  // (e.g., "en-US,en") so it can be used directly as the accept_languages
  // renderer preference. This overrides the OS/browser language preference
  // stored in profile prefs.
  const base::CommandLine& cmdline = *base::CommandLine::ForCurrentProcess();
  const char kForkAcceptLanguages[] = "dchromium-fork-accept-languages";
  if (cmdline.HasSwitch(kForkAcceptLanguages)) {
    std::string vem_lang = cmdline.GetSwitchValueASCII(kForkAcceptLanguages);
    if (!vem_lang.empty()) {
      prefs->accept_languages = vem_lang;
    }
  } else if (cmdline.HasSwitch("accept-language")) {
    // Compatibility with launchers that provide the language list through
    // the raw command line rather than the VEM switch.
    std::string command_line_languages =
        cmdline.GetSwitchValueASCII("accept-language");
    if (!command_line_languages.empty()) {
      prefs->accept_languages = command_line_languages;
    }
  } else {
