...
// Synchronizes the profile language prefs from the session's VEM profile
// before renderer and network keyed services are created. LanguagePrefs is the
// Chromium-owned source of truth for both kSelectedLanguages and the derived
// kAcceptLanguages pref.
void ApplyVirtualEnvironmentLanguagePrefs(PrefService* prefs) {
  auto* virtual_environment =
      chromium_fork::GetVirtualEnvironmentManager();
  if (!virtual_environment || !virtual_environment->is_initialized() ||
      virtual_environment->accept_languages().empty()) {
    return;
  }

  const std::vector<std::string> languages = base::SplitString(
      virtual_environment->accept_languages(), ",", base::TRIM_WHITESPACE,
...
// Applies the profile's geolocation default through Chromium's native
// permission store. This keeps Permission API state and geolocation requests
// on the same source of truth. Empty or invalid values leave the native
// profile setting untouched.
void ApplyVirtualEnvironmentGeolocationPermission(Profile* profile) {
  auto* virtual_environment =
      chromium_fork::GetVirtualEnvironmentManager();
  if (!virtual_environment || !virtual_environment->is_initialized()) {
    return;
  ...
  const std::string& configured_setting =
      virtual_environment->geo_permission_override();
  ContentSetting setting = CONTENT_SETTING_DEFAULT;
  if (configured_setting == "granted") {
    setting = CONTENT_SETTING_ALLOW;
  } else if (configured_setting == "prompt") {
    setting = CONTENT_SETTING_ASK;
  } else if (configured_setting == "denied") {
  ...
  host_content_settings_map->SetDefaultPermissionSetting(
      geolocation_type,
      permission_info->delegate().ToPermissionSetting(setting));
}
...
void ProfileImpl::OnLocaleReady(CreateMode create_mode) {
  ...
  ApplyVirtualEnvironmentLanguagePrefs(GetPrefs());
  ApplyVirtualEnvironmentGeolocationPermission(this);
