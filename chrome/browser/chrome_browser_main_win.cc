...
// === M10 Dchromium fork launch banner (startup-only, no fingerprint effect) ===
// Hard-coded strings in this block are explicitly approved by the user
// (override of CLAUDE.md #5 for the "Launching fork chrome" launch-log line).
// Every other field below is queried from the OS at runtime:
//   - username:   ::GetUserNameW()
//   - integrity:  base::win::AccessToken::IntegrityLevel() via OpenProcessToken
//   - sandbox:    inverse of --no-sandbox / --disable-sandbox /
//                 --win32k-lockdown=off on base::CommandLine::ForCurrentProcess()
//   - desktop:    hard-coded "interactive desktop" (explicit user override).
// Failure to query the OS yields an empty field; we never silently fall
// back to fake data.

const char* const kForbiddenSandboxSwitches[] = {
    sandbox::policy::switches::kNoSandbox,
    "disable-sandbox",
    "win32k-lockdown",
};

void CheckForbiddenSandboxSwitches() {
  const base::CommandLine* cmd = base::CommandLine::ForCurrentProcess();
  for (const auto* sw : kForbiddenSandboxSwitches) {
    if (cmd->HasSwitch(sw)) {
      LOG(ERROR) << "Dchromium fork: forbidden launch switch detected: "
                 << sw
                 << " (operator must remove --no-sandbox / --disable-sandbox /"
                 << " --win32k-lockdown=off before launching fork chrome)";
    }
  }
}

std::string QueryCurrentUsername() {
  wchar_t name_buf[MAX_PATH] = {};
  DWORD name_len = MAX_PATH;
  if (::GetUserNameW(name_buf, &name_len) != 0) {
    return base::WideToUTF8(name_buf);
  }
  return std::string();
}

std::string QueryIntegrityLevelString() {
  std::optional<base::win::AccessToken> access_token =
      base::win::AccessToken::FromCurrentProcess(
          /*impersonation=*/false, /*desired_access=*/TOKEN_QUERY);
  if (!access_token.has_value() || !access_token->is_valid()) {
    return std::string();
  }
  DWORD rid = access_token->IntegrityLevel();
  if (rid == MAXDWORD) {
    return std::string();
  }
...
void LogForkLaunchBanner() {
  const base::CommandLine* cmd = base::CommandLine::ForCurrentProcess();
  const bool sandbox_off =
      cmd->HasSwitch(sandbox::policy::switches::kNoSandbox) ||
      cmd->HasSwitch("disable-sandbox") ||
      cmd->HasSwitch("win32k-lockdown");
  std::string username = QueryCurrentUsername();
  std::string integrity = QueryIntegrityLevelString();

  LOG(INFO) << "Launching fork chrome (" << (username.empty() ? "?" : username)
            << " / " << (integrity.empty() ? "?" : integrity)
            << (integrity.empty() ? "" : " integrity")
            << " / sandbox " << (sandbox_off ? "OFF" : "ON")
            << " / interactive desktop)";
}

void MaybeBlockDynamicCodeForBrowserProcess() {
  // M10 fork launch banner + forbidden-switch check (startup-only, no behavior change)
  CheckForbiddenSandboxSwitches();
  LogForkLaunchBanner();
