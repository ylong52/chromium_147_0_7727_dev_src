...
// somewhere better, e.g. as a helper in first_run namespace.
  ...
  // Dchromium fork (2026-07-24): Override kApplicationLocale from VEM profile
  // (--dchromium-fork-profile-path). At this point, InitFromProfileJSON() has already
  // run (called from content_main_runner_impl.cc) and wrote --lang + --dchromium-fork-accept-languages
  // to the command line. However, ChromeCommandLinePrefStore was already constructed
  // before VEM InitFromProfileJSON(), so it missed those switches. Instead of
  // relying on command-line propagation, write kApplicationLocale directly into
  // local_state_ here so that GetApplicationLocale() reads the VEM locale instead
  // of the OS default.
  //
  // Priority: environment.locale.language > environment.regional.locale > accept_languages[0].
  {
    const base::CommandLine& cmdline = *base::CommandLine::ForCurrentProcess();
    std::string vem_locale;

    // 1. Read --lang (written by VEM InitFromProfileJSON).
    vem_locale = cmdline.GetSwitchValueASCII("lang");
    if (vem_locale.empty()) {
      // 2. Fall back to first entry of --accept-lang.
      std::string accept_lang = cmdline.GetSwitchValueASCII("accept-lang");
      if (!accept_lang.empty()) {
        size_t comma = accept_lang.find(',');
        vem_locale =
            (comma == std::string::npos)
                ? accept_lang
                : accept_lang.substr(0, comma);
      }
    }

    if (!vem_locale.empty()) {
      local_state_->SetString(language::prefs::kApplicationLocale, vem_locale);
    }
  }
#endif  // BUILDFLAG(IS_WIN)
}
