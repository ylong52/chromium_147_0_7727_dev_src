...
std::optional<std::string> CheckAndResolveLocale(std::string_view locale,
  ...
  if (lang.size() < locale.size()) {
    ...
    if (base::EqualsCaseInsensitiveASCII(lang, "es") &&
      ...
      // Desktop Chrome does not ship a standalone zh-HK locale pack. Use the
      // native Traditional Chinese UI resource fallback for Hong Kong/Macao.
      // The renderer's runtime locale remains independent and is initialized
      // from the VEM accept-languages source in renderer_main.cc.
