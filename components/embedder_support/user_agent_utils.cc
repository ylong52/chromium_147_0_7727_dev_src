...
#include "components/embedder_support/virtual_env_fwd.h"
...
#if BUILDFLAG(IS_WIN)
blink::UserAgentBrandList GetDchromiumForkBrandMajorVersionList() {
  const std::string major_version = version_info::GetMajorVersionNumber();
  return {{"Google Chrome", major_version},
          {"Not.A/Brand", "8"},
          {"Chromium", major_version}};
}

blink::UserAgentBrandList GetDchromiumForkBrandFullVersionList() {
  const std::string full_version(version_info::GetVersionNumber());
  return {{"Google Chrome", full_version},
          {"Not.A/Brand", "8.0.0.0"},
          {"Chromium", full_version}};
}

void ApplyDchromiumForkLowEntropyUAMetadata(
    blink::UserAgentMetadata* metadata) {
  metadata->brand_version_list = GetDchromiumForkBrandMajorVersionList();
  metadata->mobile = false;
  metadata->platform = "Windows";
}

void ApplyDchromiumForkHighEntropyUAMetadata(
    blink::UserAgentMetadata* metadata) {
  metadata->brand_full_version_list = GetDchromiumForkBrandFullVersionList();
  ...
  if (vem && vem->is_initialized()) {
    metadata->architecture = vem->architecture();
    metadata->model = vem->model();
    metadata->bitness = vem->bitness();
    metadata->wow64 = vem->wow64();
    metadata->platform_version = vem->platform_version();
    if (!vem->form_factors().empty()) {
      metadata->form_factors = vem->form_factors().items;
    }
  } else {
    metadata->architecture = "x86";
    metadata->model = "";
    metadata->form_factors =
        GetFormFactorsClientHint(*metadata, metadata->mobile);
...
std::string GetUnifiedPlatform() {
  ...
  // Include CPU architecture to match native Chrome UA format
  // (e.g. "Windows NT 10.0; Win64; x64" on x64 builds).
...
std::string BuildCpuInfo() {
  ...
  cpuinfo = ui::GetDeviceFormFactor() == ui::DEVICE_FORM_FACTOR_TABLET
                ...
                : "iPhone";
#elif BUILDFLAG(IS_WIN)
  // Returns "Win64; x64" on x64 builds to match native Chrome UA format.
  // GetArchitecture() returns X86_ARCHITECTURE for x64 builds on Windows,
  // but native Chrome UA uses "Win64; x64" not bare "x86".
  cpuinfo = (base::win::OSInfo::GetInstance()->GetArchitecture() ==
             base::win::OSInfo::X64_ARCHITECTURE)
                ? "Win64; x64"
                : "Win32";
...
#if BUILDFLAG(IS_WIN)
  ApplyDchromiumForkLowEntropyUAMetadata(&metadata);
#else
...
#if BUILDFLAG(IS_WIN)
  ApplyDchromiumForkHighEntropyUAMetadata(&metadata);
#else
