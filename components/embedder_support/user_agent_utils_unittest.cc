...
void CheckUserAgentStringOrdering(bool mobile_device) {
  ...
  pieces = base::SplitStringUsingSubstr(os_str, "; ", base::KEEP_WHITESPACE,
                                        base::SPLIT_WANT_ALL);
#if BUILDFLAG(IS_WIN)
  // Dchromium fork: Windows unified platform is "Windows NT 10.0" without
  // CPU architecture info, matching native US Chrome.
...
#if BUILDFLAG(IS_WIN)
  const blink::UserAgentBrandList expected_brand_version_list = {
      {"Google Chrome", major_version},
      {"Not.A/Brand", "8"},
      {"Chromium", major_version}};
  const blink::UserAgentBrandList expected_brand_full_version_list = {
      {"Google Chrome", full_version},
      {"Not.A/Brand", "8.0.0.0"},
      {"Chromium", full_version}};

  EXPECT_EQ(metadata.brand_version_list, expected_brand_version_list);
  EXPECT_EQ(metadata.brand_full_version_list, expected_brand_full_version_list);
...
#if BUILDFLAG(IS_WIN)
  EXPECT_EQ(metadata.platform_version, "19.0.0");
#elif BUILDFLAG(IS_FUCHSIA)
  EXPECT_EQ(metadata.platform_version, "");
#elif BUILDFLAG(IS_LINUX)
