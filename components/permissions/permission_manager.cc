...
// Dchromium fork: Permission override via VirtualEnvironmentManager.
...
content::PermissionResult PermissionManager::GetPermissionStatusInternal(
  ...
  // Dchromium fork: Apply VEM permission override for geolocation.
  // Sources from --dchromium-fork-permission-geolocation = granted/prompt/denied.
  // Empty VEM field = no override (default real Chromium permission flow).
  // This ensures navigator.permissions.query({name:'geolocation'}) reflects the
  // persona's permission state and stays consistent with the SetOverride'd
  // geoposition (避免 position 已返回但 permission.state=denied 这种矛盾)。
  // 必须在 device status 应用之后、return 之前；这样 override 是最末态。
  if (content_settings_type == ContentSettingsType::GEOLOCATION) {
    ...
    if (vem && vem->is_initialized()) {
      const std::string& perm_geo = vem->permission_geolocation();
      if (perm_geo == "granted") {
        result.status = PermissionStatus::GRANTED;
        result.source = content::PermissionStatusSource::UNSPECIFIED;
      } else if (perm_geo == "prompt") {
        result.status = PermissionStatus::ASK;
        result.source = content::PermissionStatusSource::UNSPECIFIED;
      } else if (perm_geo == "denied") {
        result.status = PermissionStatus::DENIED;
        result.source = content::PermissionStatusSource::UNSPECIFIED;
      }
    }
