...
// TRACE_EVENT methods on Linux or iOS builds till after we set this up.
  ...
  // Dchromium fork: VirtualEnvironmentManager initialization (must happen BEFORE
  // the renderer timezone adoptDefault below so that VEM is available).
  //
  // Browser process (process_type.empty()): read profile.json and write switches
  //   so all child processes inherit them via command-line inheritance.
  // Child processes (renderer/gpu/etc.): read switches via InitFromCommandLine().
  //
  // Order matters for fields present in BOTH CLI and JSON:
  //   1. InitFromCommandLine() first — picks up whatever the launcher put on
  //      the CLI (e.g. Go's --dchromium-fork-geo-country-code=HK).
  //   2. InitFromProfileJSON() last — JSON is the canonical session intent
  //      and overrides CLI for any field it sets. This guarantees the
  //      browser process VEM ends up with the JSON value even when the
  //      launcher disagrees with the profile.
  if (process_type.empty()) {
    // CRITICAL FIX (2026-07-23): Go launchers (interactive_launch, launch, etc.)
    // inject --dchromium-fork-geo-* switches directly on the command line.
    // InitFromCommandLine reads those into VEM first.
    chromium_fork::GetVirtualEnvironmentManager()->InitFromCommandLine();
    base::FilePath profile_path = base::CommandLine::ForCurrentProcess()
                                     ->GetSwitchValuePath(chromium_fork::switches::kForkProfilePath);
    if (!profile_path.empty()) {
      // User passed --dchromium-fork-profile-path on command line.
      // Runs AFTER InitFromCommandLine so JSON overrides CLI for any
      // overlapping field (country_code being the canonical example).
      chromium_fork::GetVirtualEnvironmentManager()->InitFromProfileJSON(profile_path);
      // Install VEM font allowlist proxy into Skia BEFORE any code calls
      // skia::DefaultFontMgr(). Must happen after VEM initialization so the
      // allowlist is available, but before font cache creation.
    }

    // Phase P0-2 (2026-07-25): Canvas Test Noise - Browser-level SessionSeed.
    // If canvas-test-noise is enabled but no --canvas-fixed-seed was supplied,
    // generate one ONCE in the browser process and append it to the current
    // process command line. This makes the SessionSeed available to the
    // renderer propagating logic (kSwitchNames in render_process_host_impl.cc),
    // so every renderer / OOPIF / worker process receives the *same* seed.
    //
    // IMPORTANT: this MUST run before any RenderProcessHost is created, i.e.
    // before the message loop starts spawning children. PreCreateThreads is
    // the right hook. The behavior is:
    //   --canvas-fixed-seed=<seed>          -> use that seed (caller-pinned)
    //   --dchromium-fork-canvas-test-noise-seed=<seed> -> legacy alias
    //   nothing                              -> generate base::RandUint64()
    //
    // If CanvasSessionSeed is disabled at the browser level, this is a no-op.
    //
    // Phase W2 (2026-07-27): also trigger when --webgl-noise-enabled=1 is
    // present. The WebGL readback hook (Phase W2) requires the same
    // cross-process session_seed stability even when canvas-test-noise is
    // not enabled, so the Browser still has to generate / propagate a seed.
    {
      base::CommandLine* cmdline = base::CommandLine::ForCurrentProcess();
      const bool noise_enabled_via_new_switch =
          cmdline->HasSwitch("canvas-test-noise-enabled");
      const bool noise_enabled_via_legacy_switch =
          cmdline->HasSwitch("dchromium-fork-canvas-test-noise-enabled");
      const bool webgl_noise_enabled =
          cmdline->HasSwitch("webgl-noise-enabled") &&
          (cmdline->GetSwitchValueASCII("webgl-noise-enabled") == "1" ||
           cmdline->GetSwitchValueASCII("webgl-noise-enabled") == "true");
      const bool has_fixed_seed =
          cmdline->HasSwitch("canvas-fixed-seed") ||
          cmdline->HasSwitch("dchromium-fork-canvas-test-noise-seed");
      if ((noise_enabled_via_new_switch || noise_enabled_via_legacy_switch ||
           webgl_noise_enabled) &&
          !has_fixed_seed) {
        // Generate the Browser-level SessionSeed and write it back to the
        // current process command line so that PropagateBrowserCommandLineToRenderer
        // can copy it into each child process' command line.
        const uint64_t seed = base::RandUint64();
        cmdline->AppendSwitchASCII("canvas-fixed-seed",
                                   base::NumberToString(seed));
        LOG(INFO) << "[CANVAS-TEST-SESSION] Browser-generated SessionSeed "
                  << "appended to command-line for child propagation: "
                  << seed;
      }
    }

    // Phase P0-2 fix (2026-07-26): initialize the CanvasTestSessionSeedManager
    // singleton AFTER the Browser-generated seed has been appended above.
    //
    // Without this call, IsEnabled() returns false (initialized_ stays false),
    // and every ApplyCanvasReadbackNoise() in getImageData/toDataURL/toBlob
    // returns false as a no-op. In practice the red solid box hash on
    // webbrowsertools.com/canvas-fingerprint/ stays byte-for-byte identical
    // to stock Chrome, even with --canvas-test-noise-enabled=1.
    //
    // Calling InitializeFromCommandLine() here is the missing hook that makes
    // §6.2 scenario A actually observably differ across profiles.
    //
    // Phase W2 (2026-07-27): also required when --webgl-noise-enabled=1 is
    // present, even if canvas-test-noise is disabled, so the WebGL hook
    // can resolve a session_seed on its own runtime gate.
    chromium_fork::CanvasTestSessionSeedManager::GetInstance()
        ->InitializeFromCommandLine();

    // After Init, re-append the resolved session_seed onto the current-process
    // command line so that child renderers receive it via
    // PropagateBrowserCommandLineToRenderer -> kSwitchNames. The
    // InitializeFromCommandLine() above might have read a value from a
    // different source (e.g. existing JSON, or a generated RandUint64) that
    // we still need to surface as --canvas-fixed-seed on the wire.
    //
    // Phase W2 (2026-07-27): also re-append when --webgl-noise-enabled=1
    // is present, regardless of canvas-test-noise enabled state. The WebGL
    // hook in the renderer process needs the seed even when canvas-test-noise
    // is disabled.
    if (process_type.empty()) {
      base::CommandLine* cmdline = base::CommandLine::ForCurrentProcess();
      const auto cfg =
          chromium_fork::CanvasTestSessionSeedManager::GetInstance()->GetConfig();
      const bool webgl_noise_enabled =
          cmdline->HasSwitch("webgl-noise-enabled") &&
          (cmdline->GetSwitchValueASCII("webgl-noise-enabled") == "1" ||
           cmdline->GetSwitchValueASCII("webgl-noise-enabled") == "true");
      if ((cfg.enabled || webgl_noise_enabled) &&
          !cmdline->HasSwitch("canvas-fixed-seed") &&
          !cmdline->HasSwitch("dchromium-fork-canvas-test-noise-seed")) {
        cmdline->AppendSwitchASCII("canvas-fixed-seed",
                                   base::NumberToString(cfg.session_seed));
        LOG(INFO) << "[CANVAS-TEST-SESSION] SessionSeed from init ("
                  << (cfg.enabled ? std::string("canvas-test-noise")
                                  : std::string("webgl-noise"))
                  << ") re-appended for child propagation: "
                  << cfg.session_seed;
      }
    }
  } else {
    // Child process: pick up switches written by the browser process.
    chromium_fork::GetVirtualEnvironmentManager()->InitFromCommandLine();

    // Phase P0-2 fix (2026-07-26): also initialize the canvas test seed
    // manager in child processes. The browser process appends --canvas-fixed-seed
    // to the cmdline before kicking off children, so this call picks up the
    // same seed via propagation. Renderer-side getImageData/toDataURL hooks
    // then use the same SessionSeed as the browser.
    chromium_fork::CanvasTestSessionSeedManager::GetInstance()
        ->InitializeFromCommandLine();
  }

  // Dchromium fork: initial ICU timezone override for renderer processes.
  // Source of truth is VEM command-line/profile data. If no persona timezone is
  // available, leave Chromium's host timezone untouched instead of inventing a
  // fallback that can split main-thread and Worker fingerprints.
  if (process_type == "renderer") {
    chromium_fork::VirtualEnvironmentManager* vem =
        chromium_fork::GetVirtualEnvironmentManager();
    if (vem && vem->is_initialized() && !vem->timezone().empty()) {
      icu::TimeZone::adoptDefault(
          icu::TimeZone::createTimeZone(vem->timezone().c_str()));
    }
  }
