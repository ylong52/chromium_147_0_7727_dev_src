...
int RendererMain(MainFunctionParams parameters) {
  ...
  // Dchromium fork: keep renderer/Worker ICU locale aligned with the regional
  // profile independently of the browser UI resource locale. Desktop Chrome
  // resolves zh-HK UI strings through zh-TW.pak, while Intl and page language
  // must retain zh-HK. Prefer the first VEM accept-language and fall back to
  // Chromium's normal --lang value when no regional profile is configured.
  std::string locale;
  if (command_line.HasSwitch(chromium_fork::switches::kForkAcceptLanguages)) {
    locale = command_line.GetSwitchValueASCII(
        chromium_fork::switches::kForkAcceptLanguages);
    const size_t comma = locale.find(',');
    if (comma != std::string::npos)
      locale.resize(comma);
  }
  if (locale.empty() && command_line.HasSwitch("accept-language")) {
    locale = command_line.GetSwitchValueASCII("accept-language");
    const size_t comma = locale.find(',');
    if (comma != std::string::npos)
      locale.resize(comma);
  }
  if (locale.empty() && command_line.HasSwitch(switches::kLang)) {
