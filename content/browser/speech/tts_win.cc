...
class TtsPlatformImplBackgroundWorker {
 ...
 private:
  ...
  // SequenceBound constructs this worker on a thread-pool sequence. That
  // sequence is not guaranteed to have COM initialized, while SAPI requires
  // a COM apartment for both enumeration and the lifetime of ISpVoice.
  // Keep the MTA alive for the entire lifetime of the SAPI objects instead of
  // using a short-lived initializer inside Initialize().
...
void TtsPlatformImplBackgroundWorker::Initialize() {
  ...
  if (!com_initializer_.Succeeded()) {
    LOG(ERROR) << "SAPI worker COM initialization failed: hr=0x"
               << std::hex << static_cast<unsigned long>(com_initializer_.hr());
  }

  const HRESULT create_hr =
      ::CoCreateInstance(CLSID_SpVoice, nullptr, CLSCTX_ALL,
                         IID_PPV_ARGS(&speech_synthesizer_));
  if (SUCCEEDED(create_hr) && speech_synthesizer_.Get()) {
    ...
    success = true;
    DVLOG(1) << "SAPI worker enumerated " << voices.size() << " native voices";
  } else {
    LOG(ERROR) << "SAPI voice initialization failed: CoCreateInstance hr=0x"
               << std::hex << static_cast<unsigned long>(create_hr);
  }
...
bool TtsPlatformImplBackgroundWorker::GetVoiceTokens(
  ...
  // Prefer the newer OneCore registry, but do not treat an empty OneCore
  // category as a successful voice discovery. Some Windows installations
  // expose the category while keeping the actual installed SAPI voices under
  // SPCAT_VOICES. In that case the legacy category is the valid fallback.
  if (SUCCEEDED(SpEnumTokens(kSPCategoryOneCoreVoices, NULL, NULL,
                             &(*out_tokens)))) {
    ULONG one_core_count = 0;
    if (SUCCEEDED((*out_tokens)->GetCount(&one_core_count)) &&
        one_core_count > 0) {
      return true;
    }
    out_tokens->Reset();
  ...
  return SUCCEEDED(
      SpEnumTokens(SPCAT_VOICES, NULL, NULL, &(*out_tokens)));
}
