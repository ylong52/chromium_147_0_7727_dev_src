...
MediaDevicesManager::BoolDeviceTypes DoCheckPermissionsOnUIThread(
  ...
  // If the bypass flag is set, return all requested device types as authorized,
  // allowing enumerateDevices() to return the full device list without explicit
  // user permission for microphone/camera.
  if (MediaDevicesPermissionChecker::ShouldBypassEnumerateDevicesPermission()) {
    MediaDevicesManager::BoolDeviceTypes result;
    for (size_t i = 0;
         i < static_cast<size_t>(MediaDeviceType::kNumMediaDeviceTypes); ++i) {
      result[i] = requested_device_types[i];
    }
    return result;
  }
...
// static
bool MediaDevicesPermissionChecker::ShouldBypassEnumerateDevicesPermission() {
  static bool bypass = base::CommandLine::ForCurrentProcess()->HasSwitch(
      "bypass-enumerate-devices-permission");
  return bypass;
}
