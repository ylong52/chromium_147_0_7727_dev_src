...
// Dchromium fork: VirtualEnvironmentManager for locale/timezone override.
...
// Dchromium fork: Geolocation override (Phase 5; 2026-07-21). Used by
// WebContentsImpl::Init() to construct a synthetic device::mojom::Geoposition
// and call device::mojom::GeolocationContext::SetOverride(). Placed outside
// the IS_ANDROID block above because Windows/Linux/macOS all need them.
...
// crbug.com/758042
...

  {
    ...
    if (vem && vem->prefers_reduced_motion()) {
...
void WebContentsImpl::Init(const WebContents::CreateParams& params,
  ...
  // Dchromium fork: Set accept_languages from VirtualEnvironmentManager.
  // This ensures navigator.languages reflects the IP-based locale setting.
  chromium_fork::VirtualEnvironmentManager* vem =
      chromium_fork::GetVirtualEnvironmentManager();
  if (vem && vem->is_initialized() && !vem->accept_languages().empty()) {
    renderer_preferences_.accept_languages = vem->accept_languages();
  }

  // Dchromium fork: Inject persona-bound geolocation into the device service
  // geolocation_context. This intercepts navigator.geolocation.getCurrentPosition()
  // at the device service level — bypassing Windows Location API entirely, mirroring
  // CDP's Emulation.setGeolocationOverride (see content/browser/devtools/protocol/
  // emulation_handler.cc:566). Empty VEM geo fields = no override (real API used).
  // 必须 lat + lon + accuracy 三个都非零才认为 override 启用；只设一部分
  // 会被 device::ValidateGeoposition 拒绝。
  if (vem && vem->is_initialized() && vem->has_geolocation_override()) {
    auto position = device::mojom::Geoposition::New();
    position->latitude = vem->geo_latitude();
    position->longitude = vem->geo_longitude();
    position->accuracy = vem->geo_accuracy();
    // altitude / heading / speed are optional. "< -10000" / "< 0" mean
    // "not reported" per the device::ValidateGeoposition contract.
    position->altitude = vem->geo_altitude();
    position->altitude_accuracy = vem->geo_altitude_accuracy();
    position->heading = vem->geo_heading();
    position->speed = vem->geo_speed();
    ...
    if (device::ValidateGeoposition(*position)) {
      ...
      GetGeolocationContext()->SetOverride(std::move(override_result));
      fprintf(stderr,
              "[FORK-DBG-GEO] SetOverride: lat=%.4f lon=%.4f acc=%.1f "
              "alt=%.1f alt_acc=%.1f heading=%.1f speed=%.1f\n",
              vem->geo_latitude(), vem->geo_longitude(), vem->geo_accuracy(),
              vem->geo_altitude(), vem->geo_altitude_accuracy(),
              vem->geo_heading(), vem->geo_speed());
      fflush(stderr);
    } else {
      fprintf(stderr,
              "[FORK-DBG-GEO] WARN: ValidateGeoposition rejected override "
              "(lat=%.4f lon=%.4f acc=%.1f); falling back to real Location API\n",
              vem->geo_latitude(), vem->geo_longitude(), vem->geo_accuracy());
      fflush(stderr);
    }
  }
