...
bool SlowWebPreferenceCache::Update() {
  ...
  if (vem) {
    if (vem->max_touch_points() >= 0) {
      pointer_events_max_touch_points_ = vem->max_touch_points();
    }
    if (vem->touch_capable() == 0) {
      touch_event_feature_detection_enabled_ = false;
      available_pointer_types_ = ui::POINTER_TYPE_FINE;
      available_hover_types_ = ui::HOVER_TYPE_HOVER;
      primary_pointer_type_ =
          blink::mojom::PointerType::kPointerFineType;
      primary_hover_type_ = blink::mojom::HoverType::kHoverHoverType;
    } else if (vem->touch_capable() == 1) {
      touch_event_feature_detection_enabled_ = true;
      available_pointer_types_ =
          ui::POINTER_TYPE_FINE | ui::POINTER_TYPE_COARSE;
      available_hover_types_ = ui::HOVER_TYPE_HOVER;
      primary_pointer_type_ =
          blink::mojom::PointerType::kPointerFineType;
      primary_hover_type_ = blink::mojom::HoverType::kHoverHoverType;
      if (pointer_events_max_touch_points_ <= 0) {
        pointer_events_max_touch_points_ = 1;
      }
    }
  }

  number_of_cpu_cores_ = base::SysInfo::NumberOfProcessors();
  if (vem && vem->is_initialized() && vem->logical_threads() > 0) {
    number_of_cpu_cores_ = vem->logical_threads();
  }
