...
void SetRuntimeFeaturesFromCommandLine(const base::CommandLine& command_line) {
  ...
  const SwitchToFeatureMap switchToFeatureMapping[] = {
      ...
      // ==========================================================================
      // REMOVED (2026-07-17 09:55): 反自动化默认值固化专项（CLAUDE.md §6 + M4 模块）
      // 目的：使 navigator.webdriver = false 成为内核硬默认值，不再依赖外部 flag。
      // 影响：以下 3 个原本会触发 AutomationControlled 特征的开关，现全部忽略。
      //       即使用户显式传入 --enable-automation / --headless / --remote-debugging-pipe，
      //       Blink 内部的 AutomationControlled 特性也不会被激活。
      // 保留：下方约 480 行的 --remote-debugging-port=0 逻辑必须保留，专用于 ChromeDriver 兼容。
      // 关联：Docs/fingerprint/_debug.log 2026-07-17 09:55 节点
      // ==========================================================================
