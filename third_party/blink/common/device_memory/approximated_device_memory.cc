...
void ApproximatedDeviceMemory::Initialize() {
  ...
  // Dchromium fork: VirtualEnvironmentManager override.
  // --dchromium-fork-memory-mb is set by InitFromProfileJSON in the browser
  // process and propagated to renderer via PropagateBrowserCommandLineToRenderer.
  // This overrides the real machine memory for navigator.deviceMemory.
  constexpr char kForkMemoryMB[] = "dchromium-fork-memory-mb";
  const base::CommandLine& cmdline = *base::CommandLine::ForCurrentProcess();
  if (cmdline.HasSwitch(kForkMemoryMB)) {
    std::string mb_str = cmdline.GetSwitchValueASCII(kForkMemoryMB);
    int64_t vem_mb = 0;
    if (base::StringToInt64(mb_str, &vem_mb) && vem_mb > 0) {
      physical_memory_mb_ = vem_mb;
    } else {
