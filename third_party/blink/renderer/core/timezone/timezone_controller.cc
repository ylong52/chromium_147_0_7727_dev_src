...
void TimeZoneController::OnTimeZoneChange(const String& timezone_id) {
  ...
  // Dchromium fork: Force timezone from virtual environment profile on all
  // renderer processes. The browser's TimeZoneMonitor sends the real OS
  // timezone via OnTimeZoneChange; we override it with VEM's configured value
  // so all JS Intl/Browser timezone APIs consistently return the virtual
  // timezone (e.g. America/New_York for en-US profile).
  base::AutoLock locker(instance().lock_);

  instance().host_timezone_id_ = timezone_id;

  if (!HasTimeZoneOverride()) {
    static constexpr char kVemTimezoneSwitch[] = "dchromium-fork-timezone";
    base::CommandLine& cmdline = *base::CommandLine::ForCurrentProcess();
    std::string vem_tz = cmdline.GetSwitchValueASCII(kVemTimezoneSwitch);
    if (!vem_tz.empty()) {
      SetIcuTimeZoneAndNotifyV8(String::FromUTF8(vem_tz));
    }
  }
}
