...
ScriptPromise<Blob> OffscreenCanvas::convertToBlob(
  ...
  if (image_bitmap) {
    ...
    // engine is intentionally NOT wired into production Blink. The
    // engine is now a standalone test-only module. No CPU-side copy
    // or pixel mutation occurs on this path; Chromium returns the
    // native StaticBitmapImage to CanvasAsyncBlobCreator unchanged.
    ...
    auto* async_creator = MakeGarbageCollected<CanvasAsyncBlobCreator>(
        image_bitmap, options, function_type, start_time, execution_context_,
        resolver);
