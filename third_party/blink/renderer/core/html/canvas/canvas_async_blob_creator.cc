...
// 2026-07-26: canvas anti-fraud noise injection (audit-revised).
...
CanvasAsyncBlobCreator::CanvasAsyncBlobCreator(
  ...
  if (image_) {
    ...
    if (skia_image_->peekPixels(&src_data_)) {
      ...
      // 2026-07-26 (audit-revised): canvas anti-fraud noise injection
      // on the toBlob / convertToBlob path.
      //
      // Apply the noise to src_data_ BEFORE the async encoder worker
      // thread picks it up. This guarantees:
      //   - audit #11/#12: toBlob and toDataURL share the same mutation
      //     (both flows see the perturbed src_data_/data_buffer pixmap).
      //   - audit #8: idempotence. Because the noise is content-free
      //     (uses only seed + canvas identity, no pixel fold-in), a
      //     second toBlob call on the same canvas mutates the SAME bits
      //     back to zero -> stable hash across readbacks.
      //   - audit #10: every intermediate is null-checked.
      if (context_) {
        const SecurityOrigin* security_origin = context_->GetSecurityOrigin();
        if (security_origin) {
          ...
          chromium_fork::ApplyCanvasReadbackNoise(src_data_, etld1_view);
        }
      }
    }
  }
