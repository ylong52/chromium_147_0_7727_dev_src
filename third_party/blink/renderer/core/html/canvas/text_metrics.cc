...
//
// Multiplies every dimensional metric by |factor|. Caller is responsible
// for producing |factor| close to 1.0 (typically from a per-text seed
// via chromium_fork::DeriveMeasureTextNoiseX).
//
// Coverage matrix (audit 2026-07-26.b):
//   * Touched (multiplied by factor): width_, actual_bounding_box_left_,
//     actual_bounding_box_right_, font_bounding_box_ascent_,
//     font_bounding_box_descent_, actual_bounding_box_ascent_,
//     actual_bounding_box_descent_, em_height_ascent_, em_height_descent_.
//   * Deliberately not touched:
//       - text_align_dx_ / baseline_y: integer-pixel grid math; rounding
//         would be a detectable fingerprint in itself.
//       - baselines_->*(): V8-exposed but not JS-iterable; perturbing
//         them adds risk without observable anti-fraud benefit.
//       - font_, direction_, text_, ctx_text_align_, ctx_text_baseline_,
//         runs_with_offset_: non-dimensional state; perturbing these
//         would corrupt the shape data used by getSelectionRects /
//         getActualBoundingBox downstream (and trigger GC trace issues
//         if RunWithOffset::x_position_ were modified in place).
//
// Idempotence: factor == 1.0 short-circuits, so callers that
// resolve noise_x = 0.0 (e.g. ShouldApplyMeasureTextNoise() returned
// false in a fallback path) do not alter metrics at all.
void TextMetrics::ShuffleMetrics(double factor) {
  if (factor == 1.0) {
    return;
  }
  width_ *= factor;
