...
// Dchromium fork: strip host ICE candidates from SDP at the point where
// SessionDescriptionPlatform objects are created. This is the final shared
// entry point for both createOffer()/createAnswer() responses (line 192) and
// setLocalDescription() callbacks (line 1864), closing the gap where
// SanitizeSdpForHostCandidates() only covered the setLocalDescription path.
// CreepJS reads pc.localDescription.sdp to fingerprint WebRTC -- without this
// filter, real host candidates leak via the createOffer promise.
std::string SanitizeSdpForHostCandidates(const std::string& sdp) {
  if (sdp.empty() || sdp.find("typ host") == std::string::npos) {
    return sdp;
  }
  std::string result;
  ...
  size_t pos = 0;
  while (pos < sdp.size()) {
    // Find next line start (a=candidate: lines begin with "a=candidate:")
    size_t candidate_pos = sdp.find("a=candidate:", pos);
    if (candidate_pos == std::string::npos) {
      // No more candidate lines; append the rest and done
      result.append(sdp.substr(pos));
      break;
    }

    // Append everything before this candidate line
    result.append(sdp.substr(pos, candidate_pos - pos));

    // Find the end of this line (\n or end of string)
    size_t line_end = sdp.find('\n', candidate_pos);
    if (line_end == std::string::npos) line_end = sdp.size();

    // Extract the line and check if it contains "typ host" (host candidate)
    std::string line = sdp.substr(candidate_pos, line_end - candidate_pos);
    if (line.find("typ host") == std::string::npos) {
      // Not a host candidate; keep it
      result.append(line);
      if (line_end < sdp.size()) result.push_back('\n');
    }
    // Host candidate: drop it silently

    pos = (line_end < sdp.size()) ? line_end + 1 : line_end;
  }
  return result;
}
...
RTCSessionDescriptionPlatform* CreateWebKitSessionDescription(
  ...
  // Dchromium fork: sanitize SDP before exposing to JS.
  std::string sanitized_sdp = SanitizeSdpForHostCandidates(sdp);
  return MakeGarbageCollected<RTCSessionDescriptionPlatform>(
      String::FromUTF8(type), String::FromUTF8(sanitized_sdp));
}
...
bool RTCPeerConnectionHandler::Initialize(
  ...
  // Dchromium fork: completely suppress ICE candidate gathering at the
  // initialization boundary. The ParseConfiguration override and
  // DidGenerateICECandidate filters may not prevent webrtc from starting its
  // ICE agent when the page calls createOffer/createDataChannel, which can
  // cause real host candidates to appear via getStats() before any filtering
  // code runs. Clearing servers and setting type=kNone here ensures the ICE
  // agent has no servers to query and no transport policy, preventing any
  // candidate generation at the source. CreepJS will see "blocked".
  configuration_.servers.clear();
  configuration_.type = webrtc::PeerConnectionInterface::kNone;
...
void RTCPeerConnectionHandler::OnIceCandidate(const String& sdp,
  ...
  // Dchromium fork: drop host candidates at the earliest possible point,
  // before RTCIceCandidatePlatform is even constructed. The ParseConfiguration
  // kNone override and DidGenerateICECandidate Type() filter may not be
  // sufficient -- webrtc can still generate host candidates in some paths.
  // This SDP-level check fires before any candidate reaches the JS layer.
  if (!sdp.empty()) {
    std::string sdp_utf8 = sdp.Utf8();
    if (sdp_utf8.find("typ host") != std::string::npos) {
      return;
    }
  }
