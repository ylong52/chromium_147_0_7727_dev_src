...
ImageData* BaseRenderingContext2D::getImageDataInternal(
  ...
  if (snapshot) {
    ...
    // Canvas anti-fraud noise injection (2026-07-26, audit-revised).
    //
    // Audit #10: every intermediate result is null-checked before use.
    // The noise gate itself (ApplyCanvasReadbackNoise) honours the
    // SessionSeedManager::IsEnabled() check (allowlist + canvas-test-
    // noise-enabled) so when the feature is disabled this is a no-op.
    if (auto* exec = GetTopExecutionContext()) {
      ...
      if (security_origin) {
        ...
        const std::string_view etld1_view(etld1_utf8);
        chromium_fork::ApplyCanvasReadbackNoise(
            image_data_pixmap, etld1_view);
      }
    }
  }
...
TextMetrics* BaseRenderingContext2D::measureText(const String& text) {
  ...
  // 2026-07-26: Canvas anti-fraud measureText noise.
  //
  // When the user opts in via --canvas-anti-fraud-seed=<n>, perturb every
  // dimensional metric by a per-text xorshift64-derived factor. This is
  // intentionally applied UNIFORMLY (not multiplied by 0 like the 060
  // stealth-coherence path) because:
  //   * users explicitly opted in,
  //   * the perturbation is small (|1.0 + noise_x| in (0.5, 1.5]),
  //   * mixing in the per-text seed makes measureText hash depend on
  //     both the canvas content AND the user's seed, so cross-process
  //     variation propagates through measureText as well.
  if (chromium_fork::ShouldApplyMeasureTextNoise()) {
    String etld1_for_seed;
    if (auto* exec = GetTopExecutionContext()) {
      const SecurityOrigin* security_origin = exec->GetSecurityOrigin();
      if (security_origin) {
        const blink::String etld1_str = security_origin->RegistrableDomain();
        etld1_for_seed =
            etld1_str.empty() ? security_origin->ToString() : etld1_str;
      }
    }
    const std::string utf8_text(text.Utf8());
    const std::string etld1_utf8 = etld1_for_seed.Utf8();
    const uint64_t site_seed =
        chromium_fork::GetAntiFraudSeed64(std::string_view(etld1_utf8));
    const double noise_x = chromium_fork::DeriveMeasureTextNoiseX(
        site_seed, std::string_view(utf8_text.c_str(), utf8_text.size()));
    metrics->ShuffleMetrics(1.0 + noise_x);
  }
