...
class MODULES_EXPORT BaseRenderingContext2D : public CanvasRenderingContext,
 ...
 protected:
  // Phase P4 (2026-07-25): PerCanvasSalt is owned by the rendering context
  // (not by a side map keyed on raw pointers) so it dies with the canvas.
  // Lazily computed on the first noise application; once derived it is
  // stable for the lifetime of the context so getImageData / toDataURL /
  // toBlob all share the same hash inputs.
  //
  // Phase P0-3 (2026-07-25): the cache is no longer needed because
  // DerivePerCanvasSalt is a pure function of (session_seed, identity,
  // algorithm_version), and identity is the address of the
  // CanvasRenderingContextHost which is stable across calls. The field is
  // retained as documentation of the design choice; do not reintroduce
  // raw-pointer maps keyed on this context.
  uint64_t canvas_test_per_canvas_salt_ = 0;
  bool canvas_test_per_canvas_salt_set_ = false;
