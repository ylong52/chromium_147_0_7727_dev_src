...
// Fork network override helpers.
//
// Command-line switches (set by VirtualEnvironmentManager::InitFromProfileJSON):
//   --dchromium-fork-network-type=wifi|ethernet|cellular|4g
//   --dchromium-fork-network-effective-type=4g|3g|slow-2g
//   --dchromium-fork-network-rtt-ms=50
//   --dchromium-fork-network-downlink-mbps=3.05
//   --dchromium-fork-network-save-data=0|1
//
// Empty / absent switch → use real NetworkStateNotifier value.

[[maybe_unused]]
static bool HasNetworkForkOverride() {
  const base::CommandLine& cmd = *base::CommandLine::ForCurrentProcess();
  return cmd.HasSwitch("dchromium-fork-network-type") ||
         cmd.HasSwitch("dchromium-fork-network-effective-type") ||
         cmd.HasSwitch("dchromium-fork-network-rtt-ms") ||
         cmd.HasSwitch("dchromium-fork-network-downlink-mbps") ||
         cmd.HasSwitch("dchromium-fork-network-save-data");
}

std::optional<WebConnectionType> GetForkConnectionType() {
  const base::CommandLine& cmd = *base::CommandLine::ForCurrentProcess();
  fprintf(stderr, "[FORK-DBG-NETINFO] GetForkConnectionType: has_switch=%d pid=%lu\n",
          cmd.HasSwitch("dchromium-fork-network-type") ? 1 : 0,
          static_cast<unsigned long>(base::GetCurrentProcId()));
  fflush(stderr);
  if (!cmd.HasSwitch("dchromium-fork-network-type")) return std::nullopt;
  std::string v = cmd.GetSwitchValueASCII("dchromium-fork-network-type");
  if (v == "wifi")    return kWebConnectionTypeWifi;
  if (v == "ethernet") return kWebConnectionTypeEthernet;
  if (v == "cellular" || v == "4g" || v == "3g") return kWebConnectionTypeCellular4G;
  if (v == "bluetooth") return kWebConnectionTypeBluetooth;
  if (v == "wimax")   return kWebConnectionTypeWimax;
  if (v == "other")   return kWebConnectionTypeOther;
  if (v == "none")    return kWebConnectionTypeNone;
  return std::nullopt;
}

std::optional<WebEffectiveConnectionType> GetForkEffectiveType() {
  const base::CommandLine& cmd = *base::CommandLine::ForCurrentProcess();
  if (!cmd.HasSwitch("dchromium-fork-network-effective-type")) return std::nullopt;
  std::string v = cmd.GetSwitchValueASCII("dchromium-fork-network-effective-type");
  if (v == "slow-2g") return WebEffectiveConnectionType::kTypeSlow2G;
  if (v == "2g")      return WebEffectiveConnectionType::kType2G;
  if (v == "3g")      return WebEffectiveConnectionType::kType3G;
  if (v == "4g")      return WebEffectiveConnectionType::kType4G;
  if (v == "offline") return WebEffectiveConnectionType::kTypeOffline;
  return std::nullopt;
}

std::optional<base::TimeDelta> GetForkHttpRtt() {
  const base::CommandLine& cmd = *base::CommandLine::ForCurrentProcess();
  if (!cmd.HasSwitch("dchromium-fork-network-rtt-ms")) return std::nullopt;
  int ms = 0;
  if (base::StringToInt(cmd.GetSwitchValueASCII("dchromium-fork-network-rtt-ms"), &ms)) {
    return base::Milliseconds(ms);
  }
  return std::nullopt;
}

std::optional<double> GetForkDownlinkMbps() {
  const base::CommandLine& cmd = *base::CommandLine::ForCurrentProcess();
  if (!cmd.HasSwitch("dchromium-fork-network-downlink-mbps")) return std::nullopt;
  double mbps = 0.0;
  if (base::StringToDouble(
          cmd.GetSwitchValueASCII("dchromium-fork-network-downlink-mbps"), &mbps)) {
    return mbps;
  }
  return std::nullopt;
}

std::optional<bool> GetForkSaveData() {
  const base::CommandLine& cmd = *base::CommandLine::ForCurrentProcess();
  if (!cmd.HasSwitch("dchromium-fork-network-save-data")) return std::nullopt;
  return cmd.GetSwitchValueASCII("dchromium-fork-network-save-data") == "1";
}
...
V8ConnectionType NetworkInformation::type() const {
  ...
  if (auto fork_type = GetForkConnectionType()) {
    fprintf(stderr, "[FORK-DBG-NETINFO] type() fork hit: %d\n", static_cast<int>(fork_type.value()));
    fflush(stderr);
    return V8ConnectionType(ConnectionTypeToEnum(fork_type.value()));
  }
...
double NetworkInformation::downlinkMax() const {
  ...
  if (!IsObserving()) {
    // Apply fork downlink_max override (kForkNetworkDownlinkMbps → both
    // max_bandwidth and downlink_throughput).
    if (auto fork_dl = GetForkDownlinkMbps()) {
      return GetNetworkStateNotifier().RoundMbps(Host(), fork_dl.value());
    }
...
V8EffectiveConnectionType NetworkInformation::effectiveType() {
  ...
  if (auto fork_ect = GetForkEffectiveType()) {
    return V8EffectiveConnectionType(EffectiveConnectionTypeToEnum(fork_ect.value()));
  }
...
uint32_t NetworkInformation::rtt() {
  MaybeShowWebHoldbackConsoleMsg();
  fprintf(stderr, "[FORK-DBG-NETINFO] rtt() called IsObserving=%d\n", IsObserving() ? 1 : 0);
  fflush(stderr);
  ...
  if (auto fork_rtt = GetForkHttpRtt()) {
    fprintf(stderr, "[FORK-DBG-NETINFO] rtt() fork override hit: %lldms\n", static_cast<long long>(fork_rtt.value().InMilliseconds()));
    fflush(stderr);
    return GetNetworkStateNotifier().RoundRtt(Host(), fork_rtt.value());
  }
...
bool NetworkInformation::saveData() const {
  ...
  if (auto fork_sd = GetForkSaveData()) {
    save_data = fork_sd.value();
  }
...
void NetworkInformation::StartObserving() {
  if (!IsObserving() && !context_stopped_) {
    // Apply fork override on initialization so the first type_ value
    // matches what type() will return (fork overrides take priority).
    if (auto fork_type = GetForkConnectionType()) {
      type_ = fork_type.value();
    } else {
