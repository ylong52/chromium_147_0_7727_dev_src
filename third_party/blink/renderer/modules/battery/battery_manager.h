...
class BatteryManager final : public EventTarget,
 ...
 private:
  ...
  // Dchromium fork (Phase 3-2): cached VEM-injected battery state. Populated
  // once at construction when VEM is initialized with battery_* switches.
  // This replaces the previous hard-coded "NotAllowedError" rejection and
  // makes the battery API return values consistent with profile.json.
  bool vem_initialized_ = false;
  bool vem_charging_ = true;
  double vem_level_ = 1.0;
  bool vem_charging_time_infinity_ = true;
  bool vem_discharging_time_infinity_ = true;
};
