...
void CanvasTestSessionSeedManager::EnsureInitializedLocked() {
  ...
  // 2. session seed — ALWAYS resolved, even when canvas-test-noise is
  //    not enabled. Reason (2026-07-27 W2 production spec):
  //      The WebGL readback hook in ReadPixelsHelper() may opt in via the
  //      runtime gate --webgl-noise-enabled=1 regardless of the Canvas 2D
  //      canvas-test-noise enable state. To make the WebGL noise output
  //      stable across the Browser / Renderer / GPU process boundary we
  //      must resolve a session_seed in every process, sourced from the
  //      same precedence chain the Browser main process used:
  //        --canvas-anti-fraud-seed=N > --canvas-fixed-seed=N
  //        > canvas_anti_fraud_seed.json > Rand
  //      When canvas-test-noise is disabled the resolver still runs but
  //      the value is consumed only by the WebGL hook (and any other
  //      future hook that opts in via its own runtime gate); the Canvas
  //      2D allowlist path ignores it.
  ...
  // 3. algorithm version (defaults to 1 when missing/invalid). Resolved
  //    unconditionally so the WebGL hook sees a stable value.
  ...
  // 5. From here on the path is canvas-test-noise-specific (origin
  //    allowlist). It only runs when canvas-test-noise is enabled.
  if (!config_.enabled) {
    initialized_ = true;
  ...
  // 6. origin allowlist (required when canvas-test-noise is enabled).
...
uint64_t CanvasTestSessionSeedManager::DetermineSessionSeedLocked(
  ...
  *out_was_overridden = false;
  ...
  // Phase 1 (anti-fraud CLI, 2026-07-27): --canvas-anti-fraud-seed=<uint64>
  // Explicit, caller-pinned override. n != 0 is honoured; n == 0 falls
  // through to legacy / JSON / Rand so we never silently pin a zero seed
  // (which would short-circuit the xorshift64 PRNG and produce constant
  // noise, itself a fingerprintable signature).
  //
  // Priority order (2026-07-27 reorder): anti-fraud CLI MUST be checked
  // BEFORE --canvas-fixed-seed because content_main_runner_impl.cc
  // appends --canvas-fixed-seed=<random> for child propagation. Without
  // this reorder the renderer's --canvas-anti-fraud-seed=<pinned> would
  // be silently overridden by the propagated random value, breaking
  // the "stable fingerprint across restart" property.
  bool caf_present = cmdline.HasSwitch(switches::kForkCanvasAntiFraudSeed);
  bool caf_valid = false;
  uint64_t caf_seed = 0;
  std::string caf_seed_str;
  if (caf_present) {
    caf_seed_str = cmdline.GetSwitchValueASCII(switches::kForkCanvasAntiFraudSeed);
    if (!base::StringToUint64(caf_seed_str, &caf_seed)) {
      LOG(ERROR) << "[canvas-anti-fraud-seed] invalid --canvas-anti-fraud-seed='"
                 << caf_seed_str << "', falling back to legacy / persistence / Rand";
      caf_valid = false;
    } else {
      caf_valid = true;
    }
  ...
  if (caf_present && caf_valid && caf_seed != 0) {
    *out_was_overridden = true;
    config_.seed_source = "cli";
    config_.seed_from_cli = true;
    config_.seed_from_persistence = false;
    config_.seed_newly_generated = false;
    config_.seed_write_back_failed = false;
    return caf_seed;
  }
  if (caf_present && caf_valid && caf_seed == 0) {
    LOG(WARNING) << "[canvas-anti-fraud-seed] CLI value is 0; "
                    "falling back to legacy / JSON / Rand";
  }

  // Phase 2 (legacy, kept for back-compat):
...
bool CanvasTestSessionSeedManager::IsEnabled() const {
  ...
  return config_.enabled && initialized_ && !config_.allowed_origins.empty();
}

bool CanvasTestSessionSeedManager::IsSeedAvailable() const {
  base::AutoLock lock(lock_);
  const_cast<CanvasTestSessionSeedManager*>(this)->EnsureInitializedLocked();
  // Allowlist presence is intentionally NOT checked here. The seed value
  // is owned by the Browser main process and propagated to children via
  // --canvas-anti-fraud-seed=<n>; if the resolver has run, the seed is
  // usable by all readback hooks, even when canvas-noise was launched
  // without an explicit origin allowlist (the WebGL production path
  // takes this branch).
  return initialized_ && config_.session_seed != 0;
}
