...
void BatteryMonitorImpl::DidChange(const mojom::BatteryStatus& battery_status) {
  ...
  fprintf(stderr, "[FORK-DBG-BATTERY] DidChange: callback_null=%d status_to_report=%d charging=%d level=%.2f\n",
          callback_.is_null() ? 1 : 0,
          status_to_report_ ? 1 : 0,
          status_.charging ? 1 : 0,
          status_.level);
  fflush(stderr);
  if (!callback_.is_null())
    ReportStatus();
