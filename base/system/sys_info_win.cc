...
// Switch name constants — duplicated here (no chromium_fork dependency) to avoid
// pulling the VEM header into base.
constexpr char kForkCpuCoresSwitch[] = "dchromium-fork-cpu-cores";
constexpr char kForkMemoryMBSwitch[] = "dchromium-fork-memory-mb";

// Returns true if --dchromium-fork-cpu-cores is set with a valid value, and
// stores the value in |out_cores|.
bool GetForkCpuCoresOverride(int* out_cores) {
  const base::CommandLine& cmdline = *base::CommandLine::ForCurrentProcess();
  if (!cmdline.HasSwitch(kForkCpuCoresSwitch)) return false;
  std::string val = cmdline.GetSwitchValueASCII(kForkCpuCoresSwitch);
  if (val.empty()) return false;
  return base::StringToInt(val, out_cores);
}

// Returns true if --dchromium-fork-memory-mb is set with a valid value, and
// stores the value in |out_mb|.
bool GetForkMemoryMBOverride(int64_t* out_mb) {
  const base::CommandLine& cmdline = *base::CommandLine::ForCurrentProcess();
  if (!cmdline.HasSwitch(kForkMemoryMBSwitch)) return false;
  std::string val = cmdline.GetSwitchValueASCII(kForkMemoryMBSwitch);
  if (val.empty()) return false;
  return base::StringToInt64(val, out_mb);
}
...
int SysInfo::NumberOfProcessors() {
  ...
  if (GetForkCpuCoresOverride(&cores_override) && cores_override > 0) {
...
ByteSize SysInfo::AmountOfTotalPhysicalMemoryImpl() {
  int64_t mb_override = 0;
  if (GetForkMemoryMBOverride(&mb_override) && mb_override > 0) {
    // 1 MB = 1 MiB for our purposes (profile.json uses decimal MB).
    // Cast to unsigned because ByteSize's constexpr ctor requires unsigned_integral
    // for runtime values (signed_integral ctor is compile-time-only).
    return MiBU(static_cast<uint64_t>(mb_override));
  }
